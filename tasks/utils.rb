...
module Utils
  class << self
    ...
    def announcement
      ...
      { 
        :subject        => "#{cfg.name} #{Snipe::VERSION} Released",
        :title          => "#{cfg.name} version #{Snipe::VERSION} has been released.",
        :urls           => "#{cfg.homepage}",
        :description    => "#{cfg.description.rstrip}",
        :release_notes  => Utils.release_notes_from(cfg.history)[Snipe::VERSION].rstrip 
      }
    end
  end
end # << self
